            ...
            .sink { [unowned self] amount, side in
                ...
                if side == self.side {
                    ...
                    case .base:
                        ...
                        self.quoteAmountString = String(describing: (decimalAmount * self.price).double.formattedString(quote))
                        self.quoteAmountDecimal = decimalAmount * self.price
                        self.baseAmountDecimal = decimalAmount
                    case .quote:
                        self.quoteAmountString = String(describing: decimalAmount.double.formattedString(quote))
