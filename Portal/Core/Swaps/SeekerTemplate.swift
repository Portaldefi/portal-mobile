...
class SeekerTemplate: ISubmarineSwap {
    ...
    func open() async throws {
        ...
        let secretHolderPublicKey = swapData.holderPubKey
        print("[SWAP] holder pubKey: \(secretHolderPublicKey)")
        let swapHash = swapData.hash
    ...
    func commit() async throws {
        ...
        let timeZero = swapData.timelock - RELATIVE_SWAP_TIMELOCK
        let paymentDeadline = timeZero + RELATIVE_PAYMENT_DEADLINE
        
        if paymentTxHeight > paymentDeadline {
            print("[SWAP] L1 payment was made late, you really shouldn't have paid the invoice, payment height: \(paymentTxHeight), payment deadline: \(paymentDeadline), timelock: \(swapData.timelock)")
        }
