//
//  PlaynetIntergrationSample.swift
//  PortalTests
//
...
@available(iOS 15.0, *)
public class PlaynetIntergrationSample {
    ...
    // EDIT ME
    static let PLAYNET_ALICE_PEER_PUBKEY_HEX = "03486d52d6c69a36a57d725a3545ca20c89e5c2c75d3bd9b7e99e7f4b658baf811"
    static let PLAYNET_ALICE_PEER_INVOICE = ""
...
//        print(help)
        ...
        print("Genesis hash: \(PlaynetIntergrationSample.bytesToHexString(bytes: genesisHash))")
        print("Genesis hash reversed: \(PlaynetIntergrationSample.bytesToHexString(bytes: reversedGenesisHash))")
        ...
        print("Chaintip hash: \(PlaynetIntergrationSample.bytesToHexString(bytes: chaintipHash))")
        print("Chaintip hash reversed: \(PlaynetIntergrationSample.bytesToHexString(bytes: reversedChaintipHash))")
        ...
        guard let lndPubkey = PlaynetIntergrationSample.hexStringToBytes(hexString: PlaynetIntergrationSample.PLAYNET_ALICE_PEER_PUBKEY_HEX) else {
            throw TestFlowExceptions.hexParsingError
        }
        guard tcpPeerHandler.connect(address: "127.0.0.1", port: 9001, theirNodeId: lndPubkey) else {
        ...
        let minedBlocks = try await rpcInterface.mineBlocks(number: 6, coinbaseDestinationAddress: fakeAddress)
        ...
        let invoiceResult = Invoice.fromStr(s: PlaynetIntergrationSample.PLAYNET_ALICE_PEER_INVOICE)
...
//        }
    ...
    private func addFundsToTestWalletIfNeeded(rpcInterface: RegtestBlockchainManager) async throws {
        ...
        if !walletNames.contains(PlaynetIntergrationSample.WALLET_NAME) {
            // if a wallet is already loaded, this will load it also
            let newWallet = try await rpcInterface.createWallet(name: PlaynetIntergrationSample.WALLET_NAME)
        ...
        let isPlaynetWalletLoaded = loadedWallets.contains(PlaynetIntergrationSample.WALLET_NAME)
        for currentWalletName in loadedWallets {
            if currentWalletName == PlaynetIntergrationSample.WALLET_NAME {
                continue
            }
        ...
        if !isPlaynetWalletLoaded {
            let _ = try await rpcInterface.loadWallet(name: PlaynetIntergrationSample.WALLET_NAME)
        }
        ...
        if walletBalance < 5 {
            ...
            let fakeAddress = try await self.getMockAddress(rpcInterface: rpcInterface)
            let _ = try await rpcInterface.mineBlocks(number: 100, coinbaseDestinationAddress: fakeAddress)
    ...
    private func getMockAddress(rpcInterface: RegtestBlockchainManager) async throws -> String {
        let scriptDetails = try await rpcInterface.decodeScript(script: PlaynetIntergrationSample.MOCK_OUTPUT_SCRIPT)
    ...
    public class MultiPeerSimulator {
        ...
        public func simulateMultiplePeers() async throws {
            ...
            */
            ...
            let reversedGenesisHash = PlaynetIntergrationSample.hexStringToBytes(hexString: reversedGenesisHashHex)!
            ...
            do {
                ...
                let connectionResult = tcpPeerHandler.connect(address: "52.50.244.44", port: 9735, theirNodeId: PlaynetIntergrationSample.hexStringToBytes(hexString: "030c3f19d742ca294a55c00376b3b355c3c90d61c6b6b39554dbc7ac19b141c14f")!)
            ...
            do {
                ...
                let connectionResult = tcpPeerHandler.connect(address: "104.196.249.140", port: 9735, theirNodeId: PlaynetIntergrationSample.hexStringToBytes(hexString: "03037dc08e9ac63b82581f79b662a4d0ceca8a8ca162b1af3551595b8f2d97b70a")!)
            ...
            do {
                ...
                let connectionResult = tcpPeerHandler.connect(address: "3.33.236.230", port: 9735, theirNodeId: PlaynetIntergrationSample.hexStringToBytes(hexString: "03864ef025fde8fb587d989186ce6a4a186895ee44a926bfc370e2c366597a3f8f")!)
            ...
            do {
                ...
                let connectionResult = tcpPeerHandler.connect(address: "52.13.118.208", port: 9735, theirNodeId: PlaynetIntergrationSample.hexStringToBytes(hexString: "02f1a8c87607f415c8f22c00593002775941dea48869ce23096af27b0cfdcc0b69")!)
            ...
            do {
                ...
                let connectionResult = tcpPeerHandler.connect(address: "69.59.18.80", port: 9735, theirNodeId: PlaynetIntergrationSample.hexStringToBytes(hexString: "03db10aa09ff04d3568b0621750794063df401e6853c79a21a83e1a3f3b5bfb0c8")!)
            ...
            do {
                ...
                let connectionResult = tcpPeerHandler.connect(address: "35.238.153.25", port: 9735, theirNodeId: PlaynetIntergrationSample.hexStringToBytes(hexString: "02816caed43171d3c9854e3b0ab2cf0c42be086ff1bd4005acc2a5f7db70d83774")!)
            ...
            do {
                ...
                let connectionResult = tcpPeerHandler.connect(address: "170.75.163.209", port: 9735, theirNodeId: PlaynetIntergrationSample.hexStringToBytes(hexString: "035e4ff418fc8b5554c5d9eea66396c227bd429a3251c8cbc711002ba215bfc226")!)
